...
def add_user():
    ...
    if cur_user_info:
        ...
        cur_id = "404_not_logged"
    form = request.form.to_dict()
    return render_template('./result.html', query_name="用户添加结果",
                           output=courier.add_user(cur_id, form['add_userid'], form['add_passwd'],
...
def modify_profile():
    ...
    if cur_user_info:
        ...
        cur_id = "404_not_logged"
    form = request.form.to_dict()
    return render_template('./result.html', query_name="用户修改结果",
                           output=courier.modify_profile(cur_id, form['modify_userid'], form['modify_passwd'],
                                                         form['modify_name'], form['modify_email'],
                                                         form['modify_privilege']))
...
def query_ticket():
    ...
    if form['ticket_priority'] == '越快越好':
        ...
        pri_str = 'cost'
    return render_template('./result.html', query_name="车票查询结果",
                           output=courier.query_ticket(form['ticket_s'], form['ticket_t'], date_str, pri_str))
...
def buy_ticket():
    ...
    return render_template('./result.html', query_name="购票结果",
                           output=courier.buy_ticket(cur_id, form['buy_trainid'], date_str, form['buy_s'],
                                                     form['buy_t'], form['buy_n'], pri_str))
...
def query_train():
    form = request.form.to_dict()
    return render_template('./result.html', query_name="火车查询结果",
                           output=courier.query_train(form['query_trainid'], form['query_date'][5:]))
...
def release_train():
    print("发行")
    form = request.form.to_dict()
    return render_template('./result.html', query_name="火车发行结果",
                           output=courier.release_train(form['release_delete_tid']))


@app.route('/delete_train', methods=['POST'])
def delete_train():
    form = request.form.to_dict()
    return render_template('./result.html', query_name="火车删除结果",
                           output=courier.delete_train(form['release_delete_tid']))
